...
public class ProjetoJogoDaForca {
	public static void main(String[] args) {
		...
		palavrasSecretas.add("espada");
		palavrasSecretas.add("cachorro");
		palavrasSecretas.add("geladeira");

		Random random = new Random();
		int tamanhoDoArrayList = palavrasSecretas.size();
		int indiceDaPalavraAleatoriaGerada = random.nextInt(tamanhoDoArrayList);
		String palavraSecreta = palavrasSecretas.get(indiceDaPalavraAleatoriaGerada); // escolhe uma palavra secreta aleatóriamente
		...
		while (!palavraFoiDescoberta && tentativas > 0) {
			...
			for (int i = 0; i < palavraSecreta.length(); i++) {
				if (palavraSecreta.charAt(i) == chute) {
					letrasDescobertas.set(i, chute); // se o chute for certo, coloco na lista de descobertas no indice correto
					acertou = true;
				}
			}
			...
			palavraFoiDescoberta = !letrasDescobertas.contains('_'); // se a palavra não contém nenhum traçinho, é pq foi completamente descoberta
		}
